...
```
The [Publish Test Results task](https://docs.microsoft.com/en-us/azure/devops/pipelines/tasks/test/publish-test-results) is used to publish the results of the regression and data validation test runs in "JUnit" format to Azure DevOps Test Plans.
...
The application package (ZIP file) containing the web app is deployed to the app service that has earlier been created by Terraform in the build stage. The [Azure Web App task](https://docs.microsoft.com/en-us/azure/devops/pipelines/tasks/deploy/azure-rm-web-app) is used to deploy the web application to Azure App Services. A public URL is registered automatically by Azure App Services for the web app to be reachable over the Internet. The URL will be used as destination (target) for the requests generated by the JMeter load tests that will be run later on.
